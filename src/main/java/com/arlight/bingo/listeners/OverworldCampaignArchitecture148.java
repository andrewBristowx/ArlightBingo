...
final class OverworldCampaignArchitecture148 {
    ...
    static final int BOSS_RITUAL_GATE_HALF_WIDTH = BOSS_GATE_CLEAR_HALF_WIDTH;
    static final int BOSS_RITUAL_GATE_HEIGHT = 8;
    ...
    static void portalSanctuaryFurnishings(List<BlockEdit> out, int cx, int y, int cz) {
        // Keep a completely empty 13x9 bay for the Nether portal. Earlier versions placed six
        // lecterns inside this exact footprint, so the portal either replaced furniture or
        // appeared visually buried between desks.
        for (int x = -6; x <= 6; x++) {
            for (int z = -4; z <= 4; z++) {
        ...
        // Furnish only the outer wall. There are deliberately no lecterns in this sanctuary.
        for (int side : new int[]{-1, 1}) {
            int x = cx + side * 9;
            // Keep the south-facing landing of both north gate towers completely empty.
            // The old center bench at z=0 overlapped step 5 of each tower entrance and the
            // auditor correctly reported "entrada de torre enterrada o sin descansillo".
            for (int z : new int[]{-6, 6}) {
                out.add(e(x, y, cz + z, Material.SPRUCE_SLAB));
                out.add(e(x - side, y, cz + z, Material.CHISELED_STONE_BRICKS));
            }
            out.add(e(cx + side * 7, y, cz + 7, Material.CHEST));
            out.add(e(cx + side * 6, y, cz + 7, Material.POLISHED_ANDESITE));
            out.add(e(cx + side * 6, y + 1, cz + 7, Material.WHITE_CANDLE));
        }
        ...
        out.add(e(cx, y, cz + 8, Material.ENDER_CHEST));
        out.add(e(cx, y, cz + 6, Material.AMETHYST_BLOCK));
        out.add(e(cx, y + 1, cz + 6, Material.AMETHYST_CLUSTER));
        for (int x : new int[]{-7, 0, 7}) {
            out.add(data(cx + x, y + 4, cz, Material.LIGHT,
    ...
    static void villageTree(List<BlockEdit> out, int x, int y, int z, int height) {
        ...
        for (int dx = -3; dx <= 3; dx++) for (int dz = -3; dz <= 3; dz++) for (int dy = 0; dy <= 3; dy++) {
            if (dx * dx + dz * dz + (dy - 1) * (dy - 1) > 11) continue;
                Material leaves = (dx + dz + dy) % 9 == 0
                        ? Material.FLOWERING_AZALEA_LEAVES : Material.OAK_LEAVES;
                String leafId = leaves == Material.FLOWERING_AZALEA_LEAVES
                        ? "flowering_azalea_leaves" : "oak_leaves";
                out.add(data(x + dx, y + height - 1 + dy, z + dz, leaves,
                        "minecraft:" + leafId
    ...
    static int bossRitualGatePlaneZ(Location ritualGate) {
        return ritualGate.getBlockZ() + 2;
    }

    /** Restores the actual locked iron-bar gate after every road and coliseum pass. */
    static List<BlockEdit> restoreClosedBossRitualGate(Location ritualGate) {
        List<BlockEdit> out = new java.util.ArrayList<>();
        int cx = ritualGate.getBlockX();
        int y = ritualGate.getBlockY();
        int z = bossRitualGatePlaneZ(ritualGate);
        int half = BOSS_RITUAL_GATE_HALF_WIDTH;

        // Five-block-deep masonry jambs close the bypasses that remained beside the bars.
        for (int side : new int[]{-1, 1}) {
            for (int lateral = half + 1; lateral <= half + 3; lateral++) {
                int x = cx + side * lateral;
                for (int depth = -2; depth <= 2; depth++) {
                    for (int yy = -1; yy <= BOSS_RITUAL_GATE_HEIGHT + 2; yy++) {
                        out.add(e(x, y + yy, z + depth, Math.floorMod(x + yy + depth, 9) == 0
        ...
        for (int x = -half; x <= half; x++) {
            for (int depth = -2; depth <= 2; depth++) {
                out.add(e(cx + x, y - 1, z + depth, Material.POLISHED_DEEPSLATE));
            }
            for (int yy = 0; yy < BOSS_RITUAL_GATE_HEIGHT; yy++) {
                out.add(data(cx + x, y + yy, z, Material.IRON_BARS,
                        "minecraft:iron_bars[east=true,north=false,south=false,"
                                + "waterlogged=false,west=true]"));
                for (int depth : new int[]{-2, -1, 1, 2}) {
                    out.add(e(cx + x, y + yy, z + depth, Material.AIR));
                }
            }
            for (int yy = BOSS_RITUAL_GATE_HEIGHT;
                 yy <= BOSS_RITUAL_GATE_HEIGHT + 2; yy++) {
                for (int depth = -2; depth <= 2; depth++) {
                    out.add(e(cx + x, y + yy, z + depth,
                            Math.floorMod(x + yy + depth, 7) == 0
    ...
    /** Rebuilds the complete boss gatehouse after roads and terrain have finished. */
    static List<BlockEdit> restoreBossGatehouse(World world, Location ritualGate) {
        List<BlockEdit> out = new java.util.ArrayList<>();
        int cx = ritualGate.getBlockX();
        int y = ritualGate.getBlockY();
        int cz = ritualGate.getBlockZ();
        bossGatehouseApron(out, world, cx, y, cz);
        buildBossGatehouseMass(out, cx, y, cz);
        tower(out, world, cx - BOSS_GATE_TOWER_OFFSET, y, cz,
                BOSS_GATE_TOWER_RADIUS, 24, Material.DARK_OAK_PLANKS, Facing.NORTH);
        tower(out, world, cx + BOSS_GATE_TOWER_OFFSET, y, cz,
                BOSS_GATE_TOWER_RADIUS, 24, Material.DARK_OAK_PLANKS, Facing.NORTH);
        return out;
    }

    private static void bossGatehouseApron(List<BlockEdit> out, World world,
                                           int cx, int y, int cz) {
        // A compact three-block belt closes the grass pockets and exposed drops beside both
        // gate towers without recreating the huge rectangular platform removed in 1.48.28.
        for (int dx = -26; dx <= 26; dx++) {
            for (int dz = -7; dz <= 10; dz++) {
                int edgeX = Math.max(0, Math.abs(dx) - 23);
                int edgeZ = dz < -4 ? -4 - dz : Math.max(0, dz - 7);
                if (Math.max(edgeX, edgeZ) > 3) continue;
                int x = cx + dx;
                int z = cz + dz;
                Material floor = Math.floorMod(dx * 7 + dz * 11, 13) < 2
                        ? Material.MOSSY_STONE_BRICKS : Material.POLISHED_ANDESITE;
                OverworldCampaignTerrain148.supportedPathCell(out, world, x, y, z, floor);
                for (int yy = y; yy <= y + 2; yy++) {
                    out.add(e(x, yy, z, Material.AIR));
                }
            }
        }
    }

    static List<BlockEdit> restoreBossRearTowers(World world, Site boss) {
        List<BlockEdit> out = new java.util.ArrayList<>();
        int y = boss.baseY() + 1;
        for (int[] rear : new int[][]{{-31, 34}, {31, 34}}) {
            tower(out, world, boss.x() + rear[0], y, boss.z() + rear[1],
                    4, 11, Material.DEEPSLATE_TILES, Facing.NORTH);
        }
        return out;
    }

    private static void buildBossGatehouseMass(List<BlockEdit> out, int cx, int y, int cz) {
    ...
    /**
     * Rebuilds the complete circular boss enclosure after every road and terrain pass. The
     * former angle-wide opening removed a large section of the coliseum; this version keeps
     * only the exact ceremonial doorway and closes every other rounded wall cell.
     */
    static List<BlockEdit> restoreBossColiseumClosure(World world, Site boss,
                                                       Location ritualGate) {
        List<BlockEdit> out = new java.util.ArrayList<>();
        int cx = boss.x();
        int y = boss.baseY() + 1;
        int cz = boss.z();
        int foundationBottom = Math.max(world.getMinHeight() + 2,
                boss.baseY() - OverworldCampaignTerrain148.WALL_FOUNDATION_DEPTH - 3);
        ...
        for (int radius = 48; radius <= 51; radius++) {
            for (int degree = 0; degree < 360; degree++) {
                ...
                int x = cx + (int) Math.round(Math.cos(angle) * radius);
                int z = cz + (int) Math.round(Math.sin(angle) * radius);
                boolean ceremonialOpening = z <= cz - 47
                        && Math.abs(x - ritualGate.getBlockX())
                        <= BOSS_GATE_CLEAR_HALF_WIDTH;
                if (ceremonialOpening) continue;

                for (int yy = foundationBottom; yy < y; yy++) {
                    out.add(e(x, yy, z, yy >= y - 5
                            ? Material.DEEPSLATE_BRICKS : Material.COBBLESTONE));
                }
                for (int yy = 0; yy < 9; yy++) {
                    Material material = Math.floorMod(x * 3 + z * 5 + yy * 7, 17) < 2
                            ? Material.MOSSY_STONE_BRICKS : Material.DEEPSLATE_BRICKS;
                    out.add(e(x, y + yy, z, material));
                }
                if (radius == 51 && degree % 7 < 4) {
                    out.add(e(x, y + 9, z, Material.DEEPSLATE_BRICK_WALL));
                }
            }
        }

        // Continuous inner patrol ledge prevents the wall from looking detached from the
        // arena floor and hides the exposed natural trench visible along several sections.
        for (int degree = 0; degree < 360; degree++) {
            double angle = Math.toRadians(degree);
            for (int radius = 44; radius <= 47; radius++) {
                int x = cx + (int) Math.round(Math.cos(angle) * radius);
                int z = cz + (int) Math.round(Math.sin(angle) * radius);
                boolean ceremonialOpening = z <= cz - 44
                        && Math.abs(x - ritualGate.getBlockX())
                        <= BOSS_GATE_CLEAR_HALF_WIDTH + 1;
                if (ceremonialOpening) continue;
                out.add(e(x, y - 1, z,
                        Math.floorMod(x + z + radius, 13) < 2
                                ? Material.MOSSY_STONE_BRICKS : Material.POLISHED_DEEPSLATE));
                for (int yy = y; yy <= y + 3; yy++) out.add(e(x, yy, z, Material.AIR));
            }
        }

        // The passage itself is restored last so gatehouse masonry and the circular wall can
        // never leave an isolated lip or an unwalkable threshold.
        int gateZ = ritualGate.getBlockZ();
        for (int z = gateZ - 8; z <= gateZ + 8; z++) {
            for (int x = ritualGate.getBlockX() - BOSS_GATE_CLEAR_HALF_WIDTH;
                 x <= ritualGate.getBlockX() + BOSS_GATE_CLEAR_HALF_WIDTH; x++) {
                out.add(e(x, y - 1, z, Math.floorMod(x + z, 11) == 0
                        ? Material.MOSSY_STONE_BRICKS : Material.POLISHED_ANDESITE));
                for (int yy = y; yy <= y + 8; yy++) out.add(e(x, yy, z, Material.AIR));
            }
            for (int side : new int[]{-1, 1}) {
                int x = ritualGate.getBlockX()
                        + side * (BOSS_GATE_CLEAR_HALF_WIDTH + 1);
                for (int yy = y - 1; yy <= y + 5; yy++) {
                    out.add(e(x, yy, z, Math.floorMod(z + yy, 11) == 0
                            ? Material.MOSSY_STONE_BRICKS : Material.DEEPSLATE_BRICKS));
                }
                out.add(e(x, y + 6, z, Material.DEEPSLATE_BRICK_WALL));
            }
        }
        return out;
    }

    static List<BlockEdit> restoreCriticalPassageLighting(Site citadel,
                                                           Location ritualGate,
                                                           Site portal) {
        List<BlockEdit> out = new java.util.ArrayList<>();
        int citadelY = citadel.baseY() + 1;
        int[][] gates = {
                {citadel.x(), citadel.z() - 55, 0, 1},
                {citadel.x(), citadel.z() + 55, 0, 1},
                {citadel.x() - 55, citadel.z() + 10, 1, 0},
                {citadel.x() + 55, citadel.z() + 10, 1, 0}
        };
        for (int[] gate : gates) {
            for (int depth : new int[]{-2, 0, 2}) {
                out.add(data(gate[0] + gate[2] * depth, citadelY + 4,
                        gate[1] + gate[3] * depth, Material.LIGHT,
                        "minecraft:light[level=14,waterlogged=false]"));
            }
        }
        ...
        int gateZ = ritualGate.getBlockZ();
        int baseRoadY = OverworldCampaignTerrain148.bossApproachWalkY(gateY, 24);
        for (int z = gateZ - 22; z <= gateZ + 12; z += 5) {
            if (Math.abs(z - gateZ) <= 1) continue;
            int progress = Math.max(0, z - (gateZ - 24));
            int walkY = z <= gateZ
                    ? baseRoadY + Math.min(gateY - baseRoadY, progress / 6)
                    : gateY;
            out.add(data(ritualGate.getBlockX(), walkY + 4, z, Material.LIGHT,
                    "minecraft:light[level=14,waterlogged=false]"));
        }

        int portalY = portal.baseY() + 1;
        for (int x : new int[]{-7, 0, 7}) {
            out.add(data(portal.x() + x, portalY + 4, portal.z(), Material.LIGHT,
                    "minecraft:light[level=14,waterlogged=false]"));
        }
        return out;
    }
