...
/** Terrain reconstruction and supported roads for the 1.47.0 Overworld campaign. */
final class OverworldCampaignTerrain147 {
    ...
    private OverworldCampaignTerrain147() { }
    ...
    private static void fillColumn(List<BlockEdit> out, World world, int x, int target, int z,
        ...
        int minFill = Math.max(world.getMinHeight() + 2, target - 40);
        boolean anchored = false;
        for (int y = target; y >= minFill; y--) {
            Material existing = world.getBlockAt(x, y, z).getType();
            if (y < target - 6 && existing.isSolid() && existing != Material.ICE) anchored = true;
            Material material;
    ...
    static List<BlockEdit> integrateFortifications(World world, Site village, Site residential,
        ...
        peninsulaBlend(out, world, portal);
        report.structures += 6;
        return out;
    }
    ...
    private static void ringSupport(List<BlockEdit> out, World world, Site site, int radius,
        ...
        for (int degree = 0; degree < 360; degree += 2) {
            ...
            for (int y = target - 1; y >= Math.max(world.getMinHeight() + 2, target - 12); y--) {
                Material existing = world.getBlockAt(x, y, z).getType();
                if (existing.isSolid() && existing != Material.WATER && y < target - 2) break;
    ...
    static List<BlockEdit> polishCoastlines(World world, Site village, Site portal, Report report) {
        List<BlockEdit> out = new ArrayList<>();
        coastline(out, world, village.x(), village.z() + 95, 85, 20);
        coastline(out, world, portal.x(), portal.z() + 10, 46, 22);
        peninsulaBlend(out, world, portal);
        report.restoredColumns += 120;
        return out;
    }

    private static void coastline(List<BlockEdit> out, World world, int cx, int cz, int radius, int depth) {
        for (int x = -radius; x <= radius; x++) for (int z = -radius; z <= radius; z++) {
            double dist = Math.sqrt(x * x + z * z);
            if (dist > radius || z < 0) continue;
            int wx = cx + x;
            int wz = cz + z;
            double blend = smooth(dist / Math.max(1.0D, radius));
            int target = SEA_LEVEL + 1 + (int) Math.round((1.0D - blend) * 6.0D) - (int) Math.round((z / (double) radius) * 4.0D);
            fillColumn(out, world, wx, target, wz, blend > 0.75D ? Material.SAND : Material.GRASS_BLOCK, false);
            for (int y = target + 1; y <= target + depth && y <= SEA_LEVEL; y++) out.add(new BlockEdit(wx, y, wz, Material.WATER));
        }
    }

    private static void peninsulaBlend(List<BlockEdit> out, World world, Site portal) {
        int startZ = portal.z() + 5;
        for (int step = 0; step <= 42; step++) {
            double t = step / 42.0D;
            int width = (int) Math.round(12 + Math.sin(Math.PI * t) * 10);
            int cz = startZ + step * 2;
            int cy = portal.baseY() - 1 - (int) Math.round(t * 5.0D);
            for (int x = -width; x <= width; x++) {
                int wx = portal.x() + x;
                Material top = Math.abs(x) > width - 2 ? Material.SAND : Material.GRASS_BLOCK;
                fillColumn(out, world, wx, cy, cz, top, false);
                if (cy <= SEA_LEVEL) {
                    for (int y = cy + 1; y <= SEA_LEVEL; y++) out.add(new BlockEdit(wx, y, cz, Material.WATER));
                }
            }
        }
    }
