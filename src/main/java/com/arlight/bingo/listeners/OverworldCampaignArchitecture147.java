...
final class OverworldCampaignArchitecture147 {
    private OverworldCampaignArchitecture147() { }
    ...
    static void garden(List<BlockEdit> out, int cx, int y, int cz, int width, int depth) {
        farmDistrict(out, cx, y, cz, width, depth, false);
    }

    static void farmDistrict(List<BlockEdit> out, int cx, int y, int cz,
                             int width, int depth, boolean orchard) {
        int hx = width / 2, hz = depth / 2;
        for (int x = -hx; x <= hx; x++) for (int z = -hz; z <= hz; z++) {
            boolean edge = Math.abs(x) == hx || Math.abs(z) == hz;
            boolean lane = x == 0 || z == 0 || (Math.abs(x) == hx / 2 && Math.abs(z) < hz - 1);
            Material base = edge ? Material.SPRUCE_PLANKS
                    : lane ? Material.DIRT_PATH
                    : orchard ? Material.GRASS_BLOCK : Material.FARMLAND;
            out.add(e(cx + x, y - 1, cz + z, base));
            for (int yy = 0; yy <= 2; yy++) out.add(e(cx + x, y + yy, cz + z, Material.AIR));
            if (edge) {
                if ((x + z) % 3 == 0) out.add(e(cx + x, y, cz + z, Material.SPRUCE_FENCE));
                continue;
            }
            if (x == 0 || Math.abs(x) == hx / 2) {
                out.add(e(cx + x, y - 1, cz + z, Material.WATER));
                continue;
            }
            if (lane) continue;
            if (orchard && Math.abs(x) % 4 == 1 && Math.abs(z) % 5 == 1) {
                out.add(e(cx + x, y, cz + z, Material.OAK_LOG));
                for (int dx = -2; dx <= 2; dx++) for (int dz = -2; dz <= 2; dz++) for (int dy = 0; dy <= 2; dy++) {
                    if (Math.abs(dx) + Math.abs(dz) + dy > 4) continue;
                    out.add(e(cx + x + dx, y + 2 + dy, cz + z + dz,
                            Math.floorMod(dx * 7 + dz * 5 + dy, 6) == 0
                                    ? Material.FLOWERING_AZALEA_LEAVES : Material.OAK_LEAVES));
                }
            } else if (!orchard) {
                Material crop = Math.floorMod(x * 17 + z * 11, 6) < 2 ? Material.CARROTS
                        : Math.floorMod(x * 13 - z * 7, 5) == 0 ? Material.BEETROOTS : Material.WHEAT;
                out.add(e(cx + x, y, cz + z, crop));
            } else if (Math.floorMod(x * 9 + z * 13, 4) == 0) {
                out.add(e(cx + x, y, cz + z, Material.POPPY));
            }
        }
        for (int gx = -hx + 2; gx <= hx - 2; gx += Math.max(6, width / 3)) {
            for (int gz : new int[]{-hz + 2, hz - 2}) {
                out.add(e(cx + gx, y, cz + gz, Material.LANTERN));
                out.add(e(cx + gx, y - 1, cz + gz, Material.SMOOTH_STONE));
            }
        }
    }
    ...
    static void fortWall(List<BlockEdit> out, int cx, int y, int cz, int radius, int height, boolean southGate) {
        ...
        if (southGate) {
            connectedPortcullis(out, cx, y, cz + radius, gateHalf, height + 2);
        }
    }
    ...
    static void gateFrame(List<BlockEdit> out, int cx, int y, int cz, int halfWidth, int height) {
        ...
        connectedPortcullis(out, cx, y, cz, Math.max(3, halfWidth - 2), height - 1);
    ...
    static void lampsAround(List<BlockEdit> out, int cx, int y, int cz, int radius, int count) {
        for (int i = 0; i < count; i++) {
            ...
            OverworldCampaignTerrain147.lamp(out, cx + (int) Math.round(Math.cos(a) * radius), y,
    ...
    static void monumentalArenaEntrance(List<BlockEdit> out, int cx, int y, int cz) {
        ...
        connectedPortcullis(out, cx, y, cz + 2, 8, 14);
        for (int x = -20; x <= 20; x++) {
            for (int z = -2; z <= 3; z++) {
