#!/usr/bin/env python3
"""Deterministic source and geometry checks for Overworld 1.48.7."""
...
require(read("BUILD_VERSION.txt").strip() == "1.48.7", "BUILD_VERSION incorrecto")
require(read("SOURCE_VERSION.txt").strip()
        == "ArlightBingo 1.48.7 BOSS-APPROACH-VILLAGE-DETAIL-AUDIT",
        "SOURCE_VERSION incorrecto")
require("<version>1.48.7</version>" in read("pom.xml"), "pom.xml no declara 1.48.7")
require("version: 1.48.7" in read("src/main/resources/plugin.yml"),
        "plugin.yml no declara 1.48.7")
for source in (builder, audit, landscape):
    require("1.48.7" in source, "una clase activa no declara 1.48.7")
require("1.48.7-boss-approach-village-detail-audit-1" in builder,
        "falta el marcador estructural 1.48.7")
