#!/usr/bin/env python3
"""Deterministic source and geometry checks for Overworld 1.48.4."""
...
require(read("BUILD_VERSION.txt").strip() == "1.48.4", "BUILD_VERSION incorrecto")
require(read("SOURCE_VERSION.txt").strip()
        == "ArlightBingo 1.48.4 TERRAIN-ROAD-BLOCKDATA-AUDIT",
        "SOURCE_VERSION incorrecto")
require("<version>1.48.4</version>" in read("pom.xml"), "pom.xml no declara 1.48.4")
require("version: 1.48.4" in read("src/main/resources/plugin.yml"),
        "plugin.yml no declara 1.48.4")
for source in (builder, audit, landscape):
    require("1.48.4" in source, "una clase activa no declara 1.48.4")
require("1.48.4-terrain-road-blockdata-audit-1" in builder,
        "falta el marcador estructural 1.48.4")
